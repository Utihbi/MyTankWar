...
class myFrame extends JFrame implements ActionListener, Runnable {
	...
	public myFrame() {
		...
		mythread.start();
		ps = new PlaySounds("rainforest.wav");
		ps.start();
	}
...
//			Recorder.setAllDefeatedEnemyNum(0);
	...
	public void run() {
		...
		while (true) {
			...
			if (! ps.getName().equals("WIN") && ! ps.getName().equals("LOSE")) {//防止重复启动声音
				...
				if (0 == Recorder.getHeroLife()) {
					...
					//
					ps = new PlaySounds("failure_tune.wav");
					ps.setName("LOSE");
					ps.start();
					// break;
				}
